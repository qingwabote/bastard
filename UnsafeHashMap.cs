...
namespace Bastard
{
        ...
        where TKey : unmanaged, IEquatable<TKey>
    {
        ...
        internal void Init(int capacity, int sizeOfValueT, int minGrowth, AllocatorManager.AllocatorHandle allocator)
        {
            ...
            Ptr = (byte*)UnsafeUtility.Malloc(totalSize, JobsUtility.CacheLineSize, allocator.ToAllocator);
        ...
        internal static HashMapHelper<TKey>* Alloc(int capacity, int sizeOfValueT, int minGrowth, AllocatorManager.AllocatorHandle allocator)
        {
            ...
            var data = (HashMapHelper<TKey>*)UnsafeUtility.Malloc(sizeof(HashMapHelper<TKey>), UnsafeUtility.AlignOf<HashMapHelper<TKey>>(), allocator.ToAllocator);
        ...
        internal static void Free(HashMapHelper<TKey>* data)
        {
            ...
            UnsafeUtility.Free(data, data->Allocator.ToAllocator);
        }
        ...
        internal void ResizeExact(int newCapacity, int newBucketCapacity)
        {
            ...
            UnsafeUtility.Free(oldPtr, Allocator.ToAllocator);
        }
        ...
        internal int Add(in TKey key, int bucket)
        {
        ...
        internal int Find(TKey key, int bucket)
        {
        ...
        where TValue : unmanaged
    {
        ...
        public ref TValue EnsureValueRef(TKey key, out bool uninitialized)
        {
            ...
            var bucket = m_Data.GetBucket(key);
            if (m_Data.AllocatedIndex > 0)
            {
                idx = m_Data.Find(key, bucket);
            }

            if (idx == -1)
            {
                idx = m_Data.Add(key, bucket);
                uninitialized = true;
            }
            else
            {
